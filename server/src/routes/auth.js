...
import { getBearerToken, mapAuthError, publicSession, publicUser } from '../lib/auth.js';
import { findUserByEmail, isEmailVerified, revokeSession } from '../lib/users.js';
import { issueVerificationCode, secondsUntilResend, verifyUserCode } from '../lib/verification.js';
...
router.post('/verify-email', async (req, res) => {
  ...
  if (!code) {
    return res.status(400).json({ error: 'Bitte geben Sie den Bestätigungscode ein.' });
  }

  const user = await findUserByEmail(email);
  if (!user) {
    return res.status(400).json({ error: 'Der Code ist ungültig oder abgelaufen.' });
  }
  ...
  try {
    await verifyUserCode(user, code);
  } catch (error) {
...
router.post('/login', async (req, res) => {
  ...
  res.json(publicSession(data.session, data.user));
});
